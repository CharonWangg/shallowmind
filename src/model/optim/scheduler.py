...
class Constant(torch.optim.lr_scheduler.ConstantLR):
    '''fix lr scheduler'''
    def __init__(self, optimizer, max_epochs=None, max_iters=None, factor=1.0, total_iters=5, last_epoch=-1, verbose=False):
...
class Step(torch.optim.lr_scheduler.StepLR):
    '''step lr scheduler'''
    def __init__(self, optimizer, step_size, max_epochs=None, max_iters=None, gamma=0.1, last_epoch=-1, verbose=False):
...
class MultiStep(torch.optim.lr_scheduler.MultiStepLR):
    '''step lr scheduler'''
    def __init__(self, optimizer, milestones, max_epochs=None, max_iters=None, gamma=0.1, last_epoch=-1, verbose=False):
...
class Linear(torch.optim.lr_scheduler.LinearLR):
    ...
    def __init__(self, optimizer, start_factor=1.0 / 3, end_factor=1.0,
                       max_epochs=5, max_iters=None, last_epoch=-1, verbose=False):
...
class CosineAnnealing(torch.optim.lr_scheduler.CosineAnnealingLR):
    ...
    def __init__(self, optimizer, max_epochs=None, max_iters=None, min_lr=0, last_epoch=-1, verbose=False):
        super(CosineAnnealing, self).__init__(optimizer=optimizer, T_max=max_iters, eta_min=min_lr,
                                                                   last_epoch=last_epoch, verbose=verbose)

@SCHEDULERS.register_module()
class OneCycle(torch.optim.lr_scheduler.OneCycleLR):
    '''one cycle annealing lr scheduler'''
    def __init__(self, optimizer, max_lr=0.1, max_iters=-1, max_epochs=-1, pct_start=0.3, anneal_strategy='cos',
        ...
        steps_per_epoch = max_iters//max_epochs + 1
        super(OneCycle, self).__init__(optimizer=optimizer, max_lr=max_lr,
                                       total_steps=max_iters, steps_per_epoch=steps_per_epoch, epochs=max_epochs,
...
class ReduceLROnPlateau(torch.optim.lr_scheduler.ReduceLROnPlateau):
    ...
    def __init__(self, optimizer, max_epochs=-1, max_iters=-1, mode='min', factor=0.1, patience=10, verbose=False,
